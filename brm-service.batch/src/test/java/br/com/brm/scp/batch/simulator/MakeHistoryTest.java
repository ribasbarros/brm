...
public class MakeHistoryTest extends AbstractTestNGSpringContextTests {
	...
	private static final int ESTOQUE_SEGURANCA = 50;

	private static final int ESTOQUE_PADRAO = 100;

	private static final int LEAD_TIME_DEFAULT = 2;

	private static final int QUANTIDADE_VENDAS_DIA = 10;
	...
	public void test() {
		...
		do {
			...
			System.out.println(String.format("DIA %s/%s/%s", calStart.get(Calendar.DATE), calStart.get(Calendar.MONTH), calStart.get(Calendar.YEAR)));
	...
	private void validaEstoqueSeguranca(SkuResponseDTO sku, Calendar calStart) {
		...
		if(sku.getEstoqueAtual() <= ESTOQUE_SEGURANCA){
			...
			doPedido(sku.getId(), calStart.getTime());
		}
		
	}

	private void doPedido(String sku, Date dateSolicitacao) {
		try {
			pedidoService.request(sku, ESTOQUE_PADRAO, dateSolicitacao, "Estoque de seguranca", false);
		} catch (PedidoOrigemNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
